<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto pt-6 pb-20 px-4 sm:px-6 lg:pt-8">
        ...
        <form action="{{route('photos.store')}}" method="post" enctype="multipart/form-data">
            ...
            <div class="my-2">
                ...
                <textarea type="text" id="description" name="description" rows="4" class="block w-full p-2 text-gray-900 border border-gray-300 rounded-md bg-white sm:text-xs focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 dark:bg-gray-800 dark:border-gray-700 dark:placeholder-gray-400 dark:text-white" placeholder="Some description here...">{{ old('description') }}</textarea>
            </div>
            <div class="my-2">
                <label class="block mb-2 text-lg font-medium text-gray-900 dark:text-white" for="image">Image file<span class="text-red-500 dark:text-red-300">*</span></label>
                <input class="block w-full p-1 text-sm text-gray-900 border border-gray-300 rounded-md cursor-pointer bg-white dark:text-gray-400 focus:outline-none dark:bg-gray-800 dark:border-gray-700 dark:placeholder-gray-400" aria-describedby="image_help" id="image" name="image" type="file">
                @error('image')
                    <div class="mt-1 text-sm text-red-500 dark:text-red-300" id="image_help">*{{ $message }}</div>
                @enderror
            </div>

            <div class="flex items-center my-4">
                <input id="checkbox" type="checkbox" value="1" name="is_private" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-indigo-500 dark:focus:ring-indigo-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 text-indigo-500 dark:text-indigo-600">
